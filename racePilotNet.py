...
tf.app.flags.DEFINE_string('train_dir', './train_log/pilotnet_v8',
...
#    cprint('Label shape' + str(label_s), 'red')
...
def jointly_loss(logits_s, logits_t, label_s, label_t, x_shape):
...
#    total_loss = loss_s + 5e-8 * (CNN_task1_l2_norm) # For task 1
    ...
    tf.summary.scalar("model/CNN_l2_norm", CNN_l2_norm)
    tf.summary.scalar("model/task1_l2_norm", task1_l2_norm)
    tf.summary.scalar("model/task2_l2_norm", task2_l2_norm)
    tf.summary.scalar('model/total_loss', total_loss)
